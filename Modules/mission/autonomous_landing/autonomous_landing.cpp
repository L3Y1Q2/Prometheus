 ...
 * Author: Lyq
 * Update Time: 2020.8.31
 * 修改说明：
 *      1. 添加distance_disarm这个判断条件,否者降落过程的逻辑不完整
 *      2. 之前代码当moving_target为true，但目标实际没动时，不能成功降落；(修复这个bug)
 *      3. 
***************************************************************************************************************************/
//ROS 头文件
#include <ros/ros.h>
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>参数读取<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    //disarm阈值
    nh.param<float>("distance_disarm", distance_disarm, 0.1);
    ...
    while (ros::ok())
    {
        ...
        if(distance_to_setpoint < distance_thres && distance_to_setpoint > distance_disarm)    //添加distance_disarm这个判断条件（lyq)
        {
            //Command_Now.Mode = prometheus_msgs::ControlCommand::Disarm;
            //如果想要无人机直接停转，需要修改底层控制逻辑。(lyq)
...
void printf_param()
{
    cout <<">>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Parameter <<<<<<<<<<<<<<<<<<<<<<<<<<<" <<endl;
    cout << "distance_thres : "<< distance_thres << endl;
    cout << "distance_disarm："<< distance_disarm <<endl;
